...
import { BookingDomainController, DisputeDomainController, AdminDomainController, CustomerDomainController } from './controllers/domainControllers.js';
import { calculateBookingFinancials, validatePayoutRequest } from './services/financialEngine.js';
import { verifyJwtToken, requireRole } from './middleware/auth.js';
import { applyCorsHeaders } from './middleware/cors.js';
import { dbUsersStore, dbOwnersStore, dbNotificationsStore, dbPropertiesStore, dbPayoutRequestsStore, dbDisputesStore } from './services/authService.js';
import { userDb, ownerDb, propertyDb, bookingDb, notificationDb, imageDb, uploadIntentDb, adminStatsDb, walletDb } from './services/dbRepository.js';
import { paymentTxDb, PaymentService, verifyPaymobHmacSha512 } from './services/paymentService.js';
import { createStorageProvider, verifyMagicBytes } from './services/storageProvider.js';
import { GLOBAL_MIN_STAY_NIGHTS, GLOBAL_MAX_STAY_NIGHTS, hasDateRangeOverlap } from './constants/bookingRules.js';
export class ExpressServerApp {
     ...
     */
    async handleHttpRequest(method, path, headers = {}, bodyPayload, searchParams) {
        const timestamp = new Date().toISOString();
        try {
            // Helper to format owner phone number
            const formatOwnerPhone = (raw, fallbackId) => {
                ...
                if (last12.length >= 10) {
                    if (last12.startsWith('20'))
                        return `+${last12}`;
                    if (last12.startsWith('0'))
                        return `+20${last12.slice(1)}`;
            ...
            if (path === '/api/v1/payments/webhook/paymob' && method === 'POST') {
                const hmacHeader = (headers['hmac'] || headers['x-paymob-hmac'] || bodyPayload?.hmac || '');
            ...
            if (path.startsWith('/api/v1/owner/')) {
                ...
                if (path.startsWith('/api/v1/owner/properties') && method === 'GET') {
                    ...
                    const ownerProperties = dbProps.length > 0 ? dbProps : Array.from(dbPropertiesStore.values()).filter(p => p.ownerId === ownerId);
                    return {
                        statusCode: 200,
                        body: {
                            success: true,
                ...
                if (path.startsWith('/api/v1/owner/properties/') && !path.includes('/images') && !path.endsWith('/archive') && !path.endsWith('/restore') && method === 'PUT') {
                    ...
                    if (bodyPayload?.title)
                        updates.title = bodyPayload.title;
                    if (bodyPayload?.unitType)
                        updates.unitType = bodyPayload.unitType;
                    if (bodyPayload?.propertyType)
                        updates.propertyType = bodyPayload.propertyType;
                    if (bodyPayload?.address)
                        updates.address = bodyPayload.address;
                    if (bodyPayload?.bedrooms !== undefined)
                        updates.bedrooms = bodyPayload.bedrooms;
                    if (bodyPayload?.bathrooms !== undefined)
                        updates.bathrooms = bodyPayload.bathrooms;
                    if (bodyPayload?.maxGuests !== undefined)
                        updates.maxGuests = bodyPayload.maxGuests;
            ...
            if (path.startsWith('/api/v1/admin/') && path !== '/api/v1/admin/auth/login') {
                ...
                if (path.includes('/documents/') && path.endsWith('/review') && method === 'POST') {
                    ...
                    const result = AdminDomainController.reviewOwnerDocument({ id: docId, status: 'PENDING' }, decision, reason);
                    return {
                        statusCode: 200,
                        body: {
                            success: true,
                ...
                if (path === '/api/v1/admin/payouts/pending' && method === 'GET') {
                    ...
                    const pageParam = searchParams instanceof URLSearchParams ? searchParams.get('page') : searchParams?.page;
                    const limitParam = searchParams instanceof URLSearchParams ? searchParams.get('limit') : searchParams?.limit;
                    let page = 1;
                    let limit = 10;
                    ...
                    const maskPaymentIdentifier = (type, accNum) => {
                        if (!accNum)
                            return '***';
                    ...
                    const eligibleItems = allPayoutRequests.filter((item) => {
                        return (item.owner?.verificationStatus === 'VERIFIED' &&
                ...
                if (path.startsWith('/api/v1/admin/disputes/') && path.endsWith('/resolve') && method === 'POST') {
                    ...
                    const validation = AdminDomainController.validateDisputeResolution(resolutionType, 5000.00, // sample frozen hold
                    disputeId.includes('split_failed') ? 3000.00 : 0.00, // sample so far released
                    refundAmount, adminNotes);
                    if (!validation.isValid) {
                        return {
                            statusCode: 400,
                            body: {
                    ...
                    if (resolutionType === 'REFUND_GUEST')
                        ledgerKey = `DISPUTE_REFUND_GUEST_${disputeId}`;
                    if (resolutionType === 'SPLIT')
                        ledgerKey = `DISPUTE_SPLIT_OWNER_RELEASE_${disputeId}`;
                ...
                if (path.startsWith('/api/v1/admin/payouts/') && path.endsWith('/process') && method === 'POST') {
                    ...
                    const result = AdminDomainController.processPayout({ id: payoutRequestId, status: 'PENDING_ADMIN_PROCESSING', grossAmount: 1000, ownerId: 'owner-001' }, action, providerTxId, rejectionReason);
                    return {
                        statusCode: 200,
                        body: {
                            success: true,
            ...
            if (path.startsWith('/api/v1/customer/')) {
                const isPublicCustomerRoute = (path === '/api/v1/customer/properties/search' && method === 'GET') ||
                ...
                if (path === '/api/v1/customer/profile' && method === 'GET') {
                    let user = await userDb.getById(customerId).catch(() => null);
                ...
                if (path === '/api/v1/customer/properties/search' && method === 'GET') {
                    ...
                    const formatted = await Promise.all(realProps.map(async (p) => {
                        const images = await imageDb.getImagesByPropertyId(p.id).catch(() => []);
                        const imageUrls = images.map((img) => img.publicUrl || img.storagePath).filter(Boolean);
                        return {
                            ...p,
                ...
                if (path === '/api/v1/customer/account/summary' && method === 'GET') {
                    ...
                    const upcomingStays = confirmedBookings.filter((b) => {
                        const checkInStr = typeof b.checkIn === 'string' ? b.checkIn : b.checkIn?.toISOString?.()?.slice(0, 10);
                        return checkInStr && checkInStr >= todayIso;
                    });
                    const totalDepositsPaid = confirmedBookings.reduce((sum, b) => sum + (Number(b.depositAmount) || 0), 0);
                    return {
                        statusCode: 200,
                        body: {
                            success: true,
                ...
                if (path.startsWith('/api/v1/customer/bookings/') && path.endsWith('/pay') && method === 'POST') {
                    const bookingId = path.split('/')[5];
                    const idempotencyKey = (headers['idempotency-key'] || headers['Idempotency-Key'] || bodyPayload?.idempotencyKey || `idemp_pay_${bookingId}_${Date.now()}`);
