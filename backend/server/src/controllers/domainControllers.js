 ...
 */
import { validateStayLength } from '../constants/bookingRules.js';
// ==========================================
// 1. PROPERTY DOMAIN CONTROLLER (RULE-4C-01 & RULE-4C-02)
// ==========================================
export class PropertyDomainController {
    static submitForReview(property) {
    ...
    static archiveProperty(property) {
    ...
    static restoreProperty(property) {
        if (property.status !== 'ARCHIVED') {
    ...
    static validateHardDelete(property, activeBookingsCount) {
...
export class BookingDomainController {
    ...
    static isWithin60MinuteModificationWindow(confirmedAtIso) {
        if (!confirmedAtIso)
            return false;
    ...
    static checkDateOverlap(checkInA, checkOutA, checkInB, checkOutB) {
...
export class DisputeDomainController {
    static createDisputeHoldPayload(dispute, ownerNetDepositEgp) {
        return {
    ...
    static checkOwnerTimeout(timeoutAtIso) {
...
export class AdminDomainController {
    static reviewOwnerDocument(doc, decision, reason) {
    ...
    static reviewProperty(prop, decision, reviewNotes) {
        if (prop.status !== 'PENDING_REVIEW') {
            throw new Error('INVALID_STATE_TRANSITION_PROP_NOT_PENDING_REVIEW');
        }
        const newStatus = decision === 'PUBLISHED' ? 'PUBLISHED' : 'DRAFT';
        return {
            ...prop,
            status: newStatus,
    ...
    static validateProviderFee(grossAmountEgp, feeEgp) {
    ...
    static validateRejectionReason(reasonCode, rejectionReason) {
    ...
    static processPayout(payout, action, providerTxId, rejectionReason) {
    ...
    static validateDisputeResolution(resolutionType, frozenHoldEgp, ownerReleasedSoFarEgp = 0, refundAmountReq, adminNotes) {
    ...
    static resolveDispute(dispute, resolutionType, refundAmount, adminNotes) {
        ...
        0, refundAmount, adminNotes);
        if (!validation.isValid) {
            throw new Error(validation.errorCode || 'INVALID_DISPUTE_RESOLUTION');
        }
        const mappedResolutionType = resolutionType === 'RELEASE_TO_OWNER' ? 'NO_FINANCIAL_ACTION' :
...
export class CustomerDomainController {
    static filterPublishedProperties(properties) {
        return properties.filter((p) => p.status === 'PUBLISHED');
    }
    static sanitizePropertyForCustomer(property) {
    ...
    static validateCustomerBookingRequest(property, checkIn, checkOut, totalGuests) {
        if (property.status !== 'PUBLISHED') {
    ...
    static cancelCustomerBooking(booking, customerId) {
