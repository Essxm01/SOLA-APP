...
async function main() {
  ...
  try {
    ...
    if (usersColsRes.rowCount === 0) throw new Error('Assertion Failed: users table was not created');
    ...
    if (ownerFkRes.rowCount === 0) throw new Error('Assertion Failed: fk_owners_users foreign key not found');
    ...
    if (!fkBlockedAsExpected) throw new Error('Assertion Failed: Foreign key constraint did not reject invalid customer_id');
