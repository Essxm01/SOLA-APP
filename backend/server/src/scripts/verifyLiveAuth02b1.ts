...
async function main() {
  ...
  try {
    ...
    if (existingOwnerRes.rowCount === 0) throw new Error('No existing owners found in production database');
    ...
    const custDecoded: any = jwt.decode(custToken);
    ...
    const ownerDecoded: any = jwt.decode(ownerToken);
