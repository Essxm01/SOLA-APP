...
export const dbUsersStore = new Map();
...
export class AuthService {
    ...
    async verifyOtp(rawPhone, code, surface, deviceInfo, ipAddress) {
        ...
        if (record.code !== code) {
            ...
            if (memRecord)
                memRecord.failedAttempts = updatedFailed;
            if (updatedFailed >= 5) {
                await otpDb.delete(canonicalPhone).catch(() => null);
                this.otpStore.delete(canonicalPhone);
                throw new Error('OTP_MAX_ATTEMPTS_EXCEEDED');
        ...
        let ownerRecord = await ownerDb.getById(user.id).catch(() => null);
        if (!ownerRecord) {
    ...
    async refreshSession(refreshToken) {
        ...
        const role = session.role || decoded.role || 'ROLE_CUSTOMER';
