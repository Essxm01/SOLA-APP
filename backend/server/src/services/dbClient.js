...
export const dbPool = {
    query: (text, params) => queryDb(text, params)
};
async function queryViaSupabaseRest(text, params, url, key) {
    const headers = {
        'apikey': key,
    ...
    if (lowerSql.includes('from users') && lowerSql.includes('phone_number = $1')) {
        ...
        if (rows.length === 0) {
            ...
            const ownerRaw = await ownerRes.json().catch(() => []);
            const ownerRows = Array.isArray(ownerRaw) ? ownerRaw : [];
    ...
    if (lowerSql.startsWith('update users') && lowerSql.includes('full_name')) {
        ...
        if (fullName !== undefined && fullName !== null && fullName !== '')
            patchBody.full_name = fullName;
        if (email !== undefined)
            patchBody.email = email || null;
        if (avatarUrl)
    ...
    if (lowerSql.includes('from otp_challenges') && lowerSql.includes('phone_number = $1')) {
        ...
        if (rows.length === 0) {
            ...
            const fallbackRaw = await fallbackRes.json().catch(() => []);
            const fallbackRows = Array.isArray(fallbackRaw) ? fallbackRaw : [];
    ...
    if (lowerSql.includes('from user_sessions') && lowerSql.includes('refresh_token_hash = $1')) {
        ...
        const rows = Array.isArray(raw) ? raw : [];
        const mapped = rows.map(s => {
            let surface = s.surface || 'CUSTOMER';
