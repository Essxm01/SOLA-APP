...
import { GLOBAL_MIN_STAY_NIGHTS, GLOBAL_MAX_STAY_NIGHTS } from '../constants/bookingRules.js';
...
export const userDb = {
    async getById(userId) {
        const res = await queryDb('SELECT id, phone_number AS "phoneNumber", phone_verified_at AS "phoneVerifiedAt", full_name AS "fullName", email, avatar_url AS "avatarUrl", status, created_at AS "createdAt", updated_at AS "updatedAt" FROM users WHERE id = $1', [userId]);
        return res.rows[0] || null;
    },
    async getByPhone(phoneNumber) {
        const res = await queryDb('SELECT id, phone_number AS "phoneNumber", phone_verified_at AS "phoneVerifiedAt", full_name AS "fullName", email, avatar_url AS "avatarUrl", status, created_at AS "createdAt", updated_at AS "updatedAt" FROM users WHERE phone_number = $1', [phoneNumber]);
        return res.rows[0] || null;
    },
    async create(user) {
        const id = user.id || crypto.randomUUID();
        const res = await queryDb(`INSERT INTO users (id, phone_number, phone_verified_at, full_name, email, avatar_url, status, created_at, updated_at)
       VALUES ($1, $2, NOW(), $3, $4, $5, COALESCE($6, 'ACTIVE'), NOW(), NOW())
       RETURNING id, phone_number AS "phoneNumber", phone_verified_at AS "phoneVerifiedAt", full_name AS "fullName", email, avatar_url AS "avatarUrl", status, created_at AS "createdAt", updated_at AS "updatedAt"`, [id, user.phoneNumber, user.fullName || null, user.email || null, user.avatarUrl || null, user.status || 'ACTIVE']);
        return res.rows[0];
    },
    async updatePhoneVerified(userId) {
        const res = await queryDb(`UPDATE users SET phone_verified_at = NOW(), updated_at = NOW()
       WHERE id = $1
       RETURNING id, phone_number AS "phoneNumber", phone_verified_at AS "phoneVerifiedAt", full_name AS "fullName", email, avatar_url AS "avatarUrl", status, created_at AS "createdAt", updated_at AS "updatedAt"`, [userId]);
        return res.rows[0] || null;
    },
    async updateProfile(userId, data) {
        const res = await queryDb(`UPDATE users
       SET full_name = COALESCE($2, full_name),
           email = CASE WHEN $3::text = '__NULL__' THEN NULL WHEN $3 IS NOT NULL THEN $3 ELSE email END,
           avatar_url = COALESCE($4, avatar_url),
           updated_at = NOW()
       WHERE id = $1
       RETURNING id, phone_number AS "phoneNumber", phone_verified_at AS "phoneVerifiedAt", full_name AS "fullName", email, avatar_url AS "avatarUrl", status, created_at AS "createdAt", updated_at AS "updatedAt"`, [userId, data.fullName || null, data.email === null ? '__NULL__' : (data.email || null), data.avatarUrl || null]);
        return res.rows[0] || null;
    }
};
// ----------------------------------------------------------------------------
...
export const ownerDb = {
    ...
    async upsert(owner) {
        ...
        await queryDb(`INSERT INTO users (id, phone_number, phone_verified_at, status, created_at, updated_at)
       VALUES ($1, $2, NOW(), COALESCE($3, 'ACTIVE'), NOW(), NOW())
       ON CONFLICT (id) DO UPDATE SET
         phone_number = EXCLUDED.phone_number,
         status = COALESCE(EXCLUDED.status, users.status),
         updated_at = NOW()`, [owner.id, owner.phoneNumber, owner.status || 'ACTIVE']).catch(() => null);
...
export const bookingDb = {
    ...
    async getBlocksByPropertyId(propertyId) {
       ...
       FROM bookings
       WHERE property_id = $1 AND status IN ('APPROVED_PENDING_PAYMENT', 'CONFIRMED')`, [propertyId]);
        if (res && res.rows) {
            return res.rows;
        }
        return [];
    },
    async getByCustomerId(customerId, phone) {
        const res = await queryDb(`SELECT b.id, b.booking_number AS "bookingNumber", b.property_id AS "propertyId", b.owner_id AS "ownerId",
       ...
       ORDER BY b.created_at DESC`, [customerId, phone || '']);
        return res.rows.map(b => {
...
export const otpDb = {
    async getByPhone(phoneNumber) {
        const res = await queryDb(`SELECT phone_number AS "phoneNumber", code, expires_at AS "expiresAt", request_count AS "requestCount", failed_attempts AS "failedAttempts", created_at AS "createdAt", updated_at AS "updatedAt"
       FROM otp_challenges WHERE phone_number = $1`, [phoneNumber]);
        return res.rows[0] || null;
    },
    async upsert(challenge) {
        const res = await queryDb(`INSERT INTO otp_challenges (phone_number, code, expires_at, request_count, failed_attempts, created_at, updated_at)
       ...
       ON CONFLICT (phone_number) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING phone_number AS "phoneNumber", code, expires_at AS "expiresAt", request_count AS "requestCount", failed_attempts AS "failedAttempts"`, [challenge.phoneNumber, challenge.code, challenge.expiresAt, challenge.requestCount, challenge.failedAttempts || 0]);
        return res.rows[0] || null;
    },
    async updateFailedAttempts(phoneNumber, failedAttempts) {
        const res = await queryDb(`UPDATE otp_challenges SET failed_attempts = $2, updated_at = NOW()
       WHERE phone_number = $1
       RETURNING phone_number AS "phoneNumber", code, expires_at AS "expiresAt", request_count AS "requestCount", failed_attempts AS "failedAttempts"`, [phoneNumber, failedAttempts]);
        return res.rows[0] || null;
    },
    async delete(phoneNumber) {
        await queryDb(`DELETE FROM otp_challenges WHERE phone_number = $1`, [phoneNumber]);
        return true;
    }
};
export const sessionDb = {
    async create(session) {
        ...
        const res = await queryDb(`INSERT INTO user_sessions (id, user_id, owner_id, surface, role, refresh_token_hash, device_info, ip_address, is_revoked, expires_at, created_at, updated_at)
    ...
    async getByRefreshTokenHash(hash) {
        const res = await queryDb(`SELECT id, user_id AS "userId", owner_id AS "ownerId", surface, role, refresh_token_hash AS "refreshTokenHash", device_info AS "deviceInfo", ip_address AS "ipAddress", is_revoked AS "isRevoked", expires_at AS "expiresAt", created_at AS "createdAt"
       FROM user_sessions WHERE refresh_token_hash = $1`, [hash]);
        return res.rows[0] || null;
    },
    async revokeByRefreshTokenHash(hash) {
        const res = await queryDb(`UPDATE user_sessions SET is_revoked = TRUE WHERE refresh_token_hash = $1`, [hash]);
        return (res.rowCount || 0) > 0;
    },
    async revokeAllForUser(userId, surface) {
        const query = surface
