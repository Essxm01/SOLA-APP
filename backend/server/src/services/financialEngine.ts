...
export interface BookingFinancialBreakdownInCents {
  totalBookingValueInCents: number;
  depositAmountInCents: number;
  solaCommissionInCents: number;
  ownerNetDepositInCents: number;
  remainingBalanceInCents: number;
  commissionOnRemainingInCents: number;
}
 ...
 */
export function calculateCommissionHalfEvenInCents(depositAmountInCents: number, commissionPercent: number = 20): number {
 ...
 */
export function roundHalfEvenInCents(cents: number): number {
  ...
  if (decimal < 0.499999999) return floor;
  if (decimal > 0.500000001) return floor + 1;
 ...
 */
export function egpToIntegerCents(egp: number): number {
...
export function calculateBookingFinancials(
  ...
  firstNightPriceEgp: number
): BookingFinancialBreakdownInCents {
  if (
    isNaN(totalBookingValueEgp) ||
...
export function validatePayoutRequest(
  ...
  actualProviderFeeEgp: number = 0
): { isValid: boolean; errorCode?: string; netAmountEgp: number } {
 ...
 */
export function validateWalletInvariants(wallet: WalletBalanceState): boolean {
  if (
