...
export async function runAuthSecuritySuite(): Promise<{ total: number; passed: number; failed: number; results: TestResult[] }> {
  ...
  try {
    const ownerPayload: JwtPayload = { sub: 'owner_123', role: 'ROLE_OWNER', iat: Date.now(), exp: Date.now() + 900 };
  ...
  try {
    const requesterOwnerId: string = 'owner_A';
    const targetResourceOwnerId: string = 'owner_B';
    const isAuthorized = (requesterOwnerId as string) === (targetResourceOwnerId as string);
