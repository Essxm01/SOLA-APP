...
export async function runCustomerFoundationSuite() {
    const results = [];
    ...
    try {
        const mockProp = { id: 'p2', status: 'DRAFT' };
    ...
    try {
        const mockProp = { status: 'PUBLISHED', maxGuests: 4, basePricePerNight: 5000 };
    ...
    try {
        const mockProp = { status: 'DRAFT', maxGuests: 4, basePricePerNight: 5000 };
    ...
    try {
        ...
        const isCustomerAOnly = Array.isArray(bookingsA) && bookingsA.every((b) => b.customerId === 'cust001');
    ...
    try {
        ...
        const isCustomerBOnly = Array.isArray(bookingsB) && bookingsB.every((b) => b.customerId === 'cust002');
