...
import { calculateBookingFinancials, validatePayoutRequest, } from '../services/financialEngine';
import { PropertyDomainController, BookingDomainController, } from '../controllers/domainControllers';
import { ExpressServerApp } from '../app';
export async function runExitAuditSuite() {
    const results = [];
    ...
    try {
        const resWithoutKey = await serverApp.handleHttpRequest('POST', '/api/v1/owner/payouts', { authorization: 'Bearer test_token' }, { amount: 1000 });
        const pass = resWithoutKey.statusCode === 400 && resWithoutKey.body?.error?.code === 'IDEMPOTENCY_KEY_REQUIRED';
    ...
    try {
        ...
        await authService.verifyOtp(phoneSingle, '123456', 'OWNER'); // First use
        // Second use with same OTP must fail
        await authService.verifyOtp(phoneSingle, '123456', 'OWNER');
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            try {
                await authService.verifyOtp(phoneLock, '999999', 'OWNER');
            }
            catch { }
        }
        // Subsequent attempt with correct code must fail because OTP was invalidated
        await authService.verifyOtp(phoneLock, '123456', 'OWNER');
    ...
    try {
        ...
        const crossPropertyAllowed = ownerA === ownerB;
        const crossBookingAllowed = ownerA === ownerB;
        const crossWalletAllowed = ownerA === ownerB;
        const crossDisputeAllowed = ownerA === ownerB;
    ...
    try {
        ...
        const isSnapshotImmutable = snapshot.propertyTitle === 'Original Title 2026' &&
