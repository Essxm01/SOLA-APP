...
try {
  ...
  globalThis.fetch = (async () => json([propertyRow('PENDING_REVIEW', 'PENDING_VERIFICATION'), propertyRow('PENDING_REVIEW', 'PENDING_VERIFICATION')])) as typeof fetch;
  await assert.rejects(
    propertyDb.updateStatusForOwner(propertyId, ownerId, 'PENDING_REVIEW', 'PENDING_VERIFICATION'),
  ...
  const statQueries = statsCalls.map((url) => new URL(url).searchParams);
  assert.ok(statQueries.some((query) => query.get('status') === 'eq.DRAFT' && query.get('verification_status') === 'eq.REJECTED'));
  assert.ok(statQueries.every((query) => query.get('status') !== 'eq.REJECTED'));
