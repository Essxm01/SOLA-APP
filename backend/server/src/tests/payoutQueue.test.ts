...
export async function runPayoutQueueSuite() {
  ...
  try {
    ...
    const unverifiedIncluded = items.some((item: any) => item.payoutRequestId.includes('unverified'));
    const completedIncluded = items.some((item: any) => item.payoutRequestId.includes('completed'));
  ...
  try {
    ...
    const items = res.body.data.items;
    const bankItem = items.find((i: any) => i.payoutMethod.methodType === 'BANK_ACCOUNT');
    const walletItem = items.find((i: any) => i.payoutMethod.methodType === 'WALLETS_EGYPT');
    const instapayItem = items.find((i: any) => i.payoutMethod.methodType === 'INSTAPAY');
    ...
    const noEstimatedNet = items.every((i: any) => i.financials.estimatedNetAmountEgp === undefined);
