...
export async function runPostgresRuntimeSuite() {
    const results = [];
    ...
    // =========================================================================
    try {
        const resTables = await pool.query(`SELECT count(*)::int as count FROM information_schema.tables WHERE table_schema = 'public'`);
        const resExt = await pool.query(`SELECT extname FROM pg_extension WHERE extname IN ('uuid-ossp', 'btree_gist')`);
    ...
    `, [testOwnerId]);
      ...
      `, [testOwnerId]);
            if (res1.rowCount && res1.rowCount > 0)
                client1Res = true;
        }
        finally {
            client1.release();
        ...
        RETURNING available_balance
      `, [testOwnerId]);
            if (res2.rowCount && res2.rowCount > 0)
                client2Res = true;
        }
        finally {
            client2.release();
        }
        const walletCheck = await pool.query(`SELECT available_balance, reserved_for_payout_balance FROM owner_wallets WHERE owner_id = $1`, [testOwnerId]);
      ...
      `, [testOwnerId]);
            ...
            const rollbackCheck = await pool.query(`SELECT available_balance FROM owner_wallets WHERE owner_id = $1`, [testOwnerId]);
