...
import { userDb } from '../services/dbRepository.js';
import { normalizePhoneNumber } from '../utils/phoneNormalizer.js';
import { verifyAccessToken } from '../services/jwtService.js';
export async function runSharedIdentityResolutionSuite() {
    const results = [];
    ...
    try {
        ...
        const isErrorCode = missingSurfaceRes.body?.error?.code === 'MISSING_OR_INVALID_AUTH_SURFACE';
        results.push({
    ...
    try {
        ...
        const isErrorCode = invalidSurfaceRes.body?.error?.code === 'MISSING_OR_INVALID_AUTH_SURFACE';
        results.push({
