...
const paymentOrigin = { type: 'PROTECTED_PAYMENT' as const, bookingId: 'booking-payment-1' };
equal(resolveCustomerAuthEntry(true, paymentOrigin, 'LOGIN').surface, 'AUTH_V2', 'protected payment uses Auth V2');
equal(resolveCustomerAuthEntry(true, paymentOrigin, 'LOGIN').intent, 'LOGIN', 'protected payment always resumes through login');
equal(resolveCustomerAuthEntry(true, paymentOrigin, 'CREATE_ACCOUNT').intent, 'LOGIN', 'protected payment cannot be redirected into account creation');
equal(cancelCustomerAuthV2(paymentOrigin).clearFavoriteHandoff, false, 'payment cancellation keeps favorite handoff untouched');
equal(cancelCustomerAuthV2(paymentOrigin).clearBookingHandoff, false, 'payment cancellation keeps booking handoff untouched');
equal(createCustomerAuthResumePermission(paymentOrigin), null, 'payment auth has no automatic resume permission');
