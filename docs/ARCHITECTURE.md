...
- `.github/workflows/auth-v2-founder-qa.yml` is the manual, workflow-dispatch-only QA gate. It runs the isolated foundation/Postgres/runtime/Worker-adapter suites, binds generated JWT/HMAC secrets plus the two Founder-provided QA secrets, deploys the QA Worker, and runs `backend`'s `qa:auth-v2-remote` acceptance script. It never publishes the production Worker.
