...
- The repository defines global booking-night bounds of 2–30 nights. Other cancellation/refund policy must be verified task-by-task; no complete current-policy summary is recorded here.
...
- Pending-to-available release is not implemented by the current payment completion path. Payout action must not be made eligible solely because pending funds exist; repository behavior uses a 500 EGP minimum for available payout eligibility.
...
- Full cancellation/refund matrix and remaining-balance payment policy are not safely derivable as complete product rules from the current memory documents.
- Real Owner account creation/KYC onboarding is intentionally not implemented in the Owner first-run flow.
