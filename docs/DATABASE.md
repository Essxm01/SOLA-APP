...
The retained repository migration history begins at `backend/database/migrations/008_*.sql`. Migrations `021_harden_critical_rpc_privileges.sql`, `022_identity_session_persistence_integrity.sql`, and `023_finalize_identity_session_persistence.sql` are part of the P14.1/P1.2 published and live-verified baseline (`5decd03f…` / `92dc3916…`). Migration `024_atomic_property_media_commit.sql` is part of the published P1.3 baseline (`fb38414…`) and is verified applied live in Supabase (with `public.konfrm_commit_property_media` and `property_images_one_active_per_upload_intent_idx` active). Migration `025_availability_blocking_integrity.sql` (P1.4 availability blocking integrity and cross-table serialization) and Migration `026_atomic_booking_request_creation.sql` (P1.5 atomic booking request creation RPC `konfrm_create_booking_request`) are published and live. Migration `027_wallet_ledger_append_only.sql` is the current P1.6 repository-only candidate and is NOT_APPLIED_LIVE. This is **not** a complete historical baseline: `000_schema_baseline` exists only in the live application ledger, and several retained effects exist without a corresponding application-ledger record. Do not infer missing history, RLS policies, or intended grants from filenames alone.
...
| Money | payment transactions, owner wallets/ledger, payout methods/requests | Payment, upload, payout, and ledger identifiers carry uniqueness/idempotency protections. Under candidate migration `027` (NOT_APPLIED_LIVE), `wallet_ledger_entries` financial history is protected against UPDATE/DELETE/TRUNCATE, preserving only the existing booking FK `ON DELETE SET NULL` referential-null transition as the narrow non-financial exception. |
...
- `wallet_ledger_entries` financial history is protected against UPDATE, DELETE, and TRUNCATE under candidate migration `027` (NOT_APPLIED_LIVE), with the existing booking FK `ON DELETE SET NULL` referential-null transition preserved as the narrow non-financial exception (requiring `pg_trigger_depth() > 1` and all non-booking fields identical).
