...
| Auth V2 QA Worker | Founder-only runtime validation for Customer challenge/registration routes | `backend/wrangler.auth-v2-qa.json`; requires explicit matching `SUPABASE_PROJECT_REF` + QA `SUPABASE_URL`, no `DATABASE_URL`, the QA `SUPABASE_SERVICE_ROLE_KEY`, `AUTH_V2_ENABLED`, `AUTH_ENVIRONMENT=founder_qa`, `AUTH_DELIVERY_MODE=DEVELOPMENT_FIXED_OTP`, and secret bindings (`AUTH_DEVELOPMENT_OTP`, `AUTH_OTP_HMAC_SECRET`, JWT secrets). Never point it at production or expose the fixed OTP. |
