...
| RLS / public RPC exposure | Broken / remediation prerequisite | `LIVE_DB_OBSERVED` | All public app tables are RLS-enabled/no-policy; backend uses service role. But four critical KONFRM SECURITY DEFINER RPCs plus `rls_auto_enable` are executable by anon/authenticated despite migration 019/020 intent. Route P14.1 before P1.2. |
