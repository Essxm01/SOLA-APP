...
| DC-13 | Critical RPC privilege intent versus live configuration | Retained migrations 019/020 revoke public execution and grant `service_role` only | P1.1 live metadata: anon/authenticated can execute all critical SECURITY DEFINER payment/KYC/registration functions | Live metadata is a security configuration contradiction, not a product decision. Do not invoke mutators; remediate only in approved P14.1. | Payment, KYC, registration, RLS assurance | Yes—before P1.2 | No |
