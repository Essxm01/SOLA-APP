...
- **Stop Conditions:** Agents must STOP and ask only when hitting:
  1. An unapproved Product, Business, Financial, or Architectural policy change.
  2. A requirement for destructive, unverified, or live mutations (live Supabase/Storage/Cloudflare).
  3. Truly exhausted safe autonomy or an unresolvable external blocker.

---

## 2. Resource Management & Agent Delegation Model

- **Strategic Brain Authority:** The ChatGPT KONFRM Project is the highest strategic brain, product bible, and Founder interface.
- **Zero New Paid API/AI Spend:** No additional paid AI tokens, API keys, or cloud spend are approved. Work must be executed strictly within existing subscription quotas.
...
- **Phase 4 Entry Gate:** Phase 4 (Unified Design System) and subsequent UI/UX phases represent a formal design program entry where the Live Access Protocol (LAP) joins. No agent may autonomously dispatch or execute Phase 4 tasks without explicit Founder authorization and a designated design-system operating model.
