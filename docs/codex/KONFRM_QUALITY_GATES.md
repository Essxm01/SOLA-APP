...
1. **Context compliance:** mandatory core refresh occurred before implementation; applicable domain authorities and open decisions were checked.
2. **Universal DoD / intent:** outcome and authority are traceable; no known in-scope placeholder/fake behavior remains.
3. **API and DB/data:** canonical source, validation, authorization, persistence, migration/RPC/RLS impact, and truthful failure behavior are proven.
...
Immediately before closure, re-read the active task contract, `KONFRM_MASTER_RULES.md`, relevant domain authority, and these applicable gates. Resolve divergence, retest, then include the required Context Compliance Evidence in the phase report. A build, CI result, or conformity to existing code alone is not closure evidence.
