...
- prepares (but does **not** apply) migration `024_atomic_property_media_commit.sql`, with one explicit transaction, a narrow `SECURITY INVOKER` `konfrm_commit_property_media` RPC, fixed search path, explicit service-role-only execute privilege, schema-migration ledger entry, and a partial unique index enforcing one active image per upload intent;
- makes Worker media commit invoke that RPC rather than compose an image INSERT and upload-intent PATCH;
- locks the upload intent before its active-image lookup; a replay returns the canonical image only when both the intent is `COMMITTED` and the matching image is `ACTIVE`, while contradictory half-states, wrong owner/property/object, expired intent, and RPC failure return no claimed success;
...
- mutually separates ordinary draft (`DRAFT + UNVERIFIED/PENDING`) and rejected draft (`DRAFT + REJECTED`) in Owner summary counts;
...
- prevents DB and query outages from masquerading as HTTP 404 in Owner and Admin property routes (presigned URL, image commit, image reads, delete, archive, restore, Admin detail, Admin approve, Admin review);
- normalizes upload MIME types to canonical lowercase before upload intent creation and binding validation;
- adds explicit `::text` result casts to `konfrm_commit_property_media` in migration 024 to prevent `varchar`/`text` PL/pgSQL return structure mismatches;
- keeps Finding 5 (Admin pending queue oldest-first ordering) deferred as out-of-scope for P1.3, to be addressed in future Admin property review UX work;
