...
export const PropertiesView: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<PropertyStatus | 'ALL' | 'REJECTED'>('ALL');
  ...
  const filteredProperties = properties.filter((p) => {
    ...
    // Status filter
    const matchesStatus = activeFilter === 'ALL' || (activeFilter === 'REJECTED' ? isRejectedProperty(p) : p.status === activeFilter && !(activeFilter === 'DRAFT' && isRejectedProperty(p)));

    return matchesSearch && matchesStatus;
  });

  const filterTabs: { id: PropertyStatus | 'ALL' | 'REJECTED'; label: string; count?: number }[] = [
