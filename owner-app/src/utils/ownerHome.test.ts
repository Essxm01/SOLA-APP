...
const run = () => {
  const equal = (actual: unknown, expected: unknown, message?: string) => { if (actual !== expected) throw new Error(message || `Expected ${String(expected)} but received ${String(actual)}`); };
  ...
  const propertySummary = getPropertyHomeSummary([{ status: 'PENDING_REVIEW' }, { status: 'DRAFT', verificationStatus: 'REJECTED' }, { status: 'DRAFT', verificationStatus: 'UNVERIFIED' }, { status: 'PUBLISHED' }] as any);
  equal(propertySummary.pendingReview, 1);
  equal(propertySummary.drafts, 1, 'a rejected draft is not also counted as a normal draft');
