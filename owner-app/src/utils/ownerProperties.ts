...
export const isRejectedProperty = (property: Property) => property.status === 'DRAFT' && property.verificationStatus === 'REJECTED';
export const isPropertyActionRequired = (property: Property) => property.status === 'DRAFT';
...
export const getOwnerPropertyCollections = (properties: Property[], filter: OwnerPropertyFilter) => {
  ...
  return matching.sort((a, b) => Number(isRejectedProperty(b)) - Number(isRejectedProperty(a)) || statusOrder[a.status] - statusOrder[b.status] || String(b.updatedAt).localeCompare(String(a.updatedAt)));
};
